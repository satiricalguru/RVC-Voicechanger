...
export function Sidebar({
  ...
  // Dynamic mapping with translation lookup
  const localizedNav = NAV.map((item) => ({
    ...item,
    label: t(item.id),
  }))

  const localizedStaticCollections = STATIC_COLLECTIONS.map((c) => {
    let key: keyof typeof TRANSLATIONS.en = "allVoices"
    if (c.id === "Favorites") key = "favorites"
    if (c.id === "My Voices") key = "myVoices"
    return {
      ...c,
      label: t(key),
    }
    ...
    <aside className="flex h-full w-60 shrink-0 flex-col gap-6 border-r border-white/10 bg-black px-3 py-4">
              ...
              <div key={item.id} className="flex flex-col">
                ...
                {item.id === "voicelab" && (
                  ...
                  >
                    ...
                    <span className="font-medium">{t("importRvc")}</span>
                  </button>
                )}
              </div>
            )
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-0.5">
          {localizedStaticCollections.map((c) => {
            const Icon = c.icon
      ...
      <div className="mt-auto flex flex-col gap-2 rounded-lg border border-white/10 bg-white/[0.02] p-3">
        ...
        <Stat icon={Cpu} label={t("backend")} value={mode === "ai" ? "RVC AI" : "DSP"} />
        <Stat icon={Zap} label={t("latency")} value={`${latencyMs.toFixed(0)} ms`} />
        <Stat icon={Waveform} label={t("chunk")} value={`${chunkMs} ms`} />
        <div className="mt-1 truncate font-mono text-[10px] text-white/40">
          <span className="text-white/30">{t("voice")}:</span>{" "}
